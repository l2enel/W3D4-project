def it_was_ok
  ...
  Movieselect(:id, :title, :score).where(score: 2..3)
...
def harrison_ford
  ...
  Movies.select(:id, :title).joins(:actors { name: 'Harrison Ford' } )
...
def biggest_cast
  ...
  Movies.select(:id, :title).joins(:actors).group('movies.id')
  .order('COUNT(movies.id) DESC').limit(3)
end
...
def directed_by_one_of(them)
  ...
  Movies.select(:id, :title).joins(:director).where(actors: { name: them })
end
...
def movie_names_before_1940
  ...
  Movies.where('yr < 1940').pluck(:title)
end
